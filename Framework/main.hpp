...
int _main_(MainArgs args)
{
    ...
    dObj->LoadMesh("Ln.obj");
    dObj->LoadTexture("pink.tga");
...
int MainCallback(CallbackArgs args)
{
    ...
    if (Keyboard::GetKeyDown(args, 'W'))
    {
        dObj->Transform.Z += 0.5;
    }
    if (Keyboard::GetKeyDown(args, 'S'))
    {
        dObj->Transform.Z -= 0.5;
    }
    if (Keyboard::GetKeyDown(args, VK_SPACE)) {
        texture();
    }
    return 1;
}
